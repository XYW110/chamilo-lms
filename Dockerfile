...
WORKDIR /app
COPY . .
RUN composer install --no-dev --no-interaction --optimize-autoloader --ignore-platform-reqs 2>&1 | tail -20
...
RUN docker-php-ext-configure gd --with-freetype --with-jpeg && docker-php-ext-install -j4 intl gd pdo_mysql zip exif ldap
